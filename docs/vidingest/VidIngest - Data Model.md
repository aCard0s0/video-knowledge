...
| `phase_updated_at` | TIMESTAMP | nullable | Timestamp of last phase update |
...
| `downloaded_at` | TIMESTAMP | nullable | When the video was downloaded |
...
**Indexes**: source, channel_name, status, pipeline_run_id, metadata (GIN)
...
| `last_sync_attempt_at` | TIMESTAMP | nullable | Last sync attempt time |
| `last_sync_success_at` | TIMESTAMP | nullable | Last successful sync time |
...
| `published_at` | TIMESTAMP | nullable | Publish date if present (`upload_date`/`timestamp`) |
...
| `first_seen_at` | TIMESTAMP | NOT NULL | When this entry first appeared in our catalog |
| `last_seen_at` | TIMESTAMP | NOT NULL | When this entry was last observed during sync |
| `created_at` | TIMESTAMP | NOT NULL | Set on persist |
| `updated_at` | TIMESTAMP | NOT NULL | Updated on every save |
...
**Indexes**: transcription_id, (start_seconds, end_seconds)
...
**Indexes**: video_id, embedding (IVFFlat with cosine distance)
...
(default 30s with 5s overlap). `speaker_ids` is a PostgreSQL `uuid[]` array; `transcript_text`
and `ocr_text` are independently nullable.
