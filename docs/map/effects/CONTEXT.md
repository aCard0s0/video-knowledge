...
| an **LLM provider** | `vidingest.search.embeddings.provider` / `vidingest.knowledge.provider`. Provider-named classes speak that wire protocol; every neutral surface is named `llm` |
...
- `compose/infra/infra.yml` (lines 102, 140) → `compose/infra/paddleocr-server/`, `compose/infra/diarize-asr/`
